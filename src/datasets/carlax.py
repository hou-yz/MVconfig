...
class CarlaX(VisionDataset):
    def __init__(self, opts, root=os.path.expanduser("~/Data/CarlaX"), port=2000, tm_port=8000, euler2vec='none'):
        ...
        self.env = CarlaCameraSeqEnv(opts, port=port, tm_port=tm_port, euler2vec=euler2vec)
        self.img_shape = [IMAGE_H, IMAGE_W]  # H,W
        x_min, x_max, y_min, y_max = opts["spawn_area"]
    ...
    def get_worldgrid_from_worldcoord(self, world_coord):
        ...
        x_min, x_max, y_min, y_max = self.env.opts["spawn_area"]
